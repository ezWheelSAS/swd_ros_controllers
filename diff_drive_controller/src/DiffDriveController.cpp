    ...
    : m_nh(nh)
{
    ...
    // Init
    if(m_leftMotorInitialized) m_clientLeft.getOdometry(m_dLeft_prev);    // en mm
...
void DiffDriveController::cbTimerOdom()
{
    ...
    if(m_leftMotorInitialized) m_clientLeft.getOdometry(dLeft_now);    // en mm
    if(m_rightMotorInitialized) m_clientRight.getOdometry(dRight_now); // en mm
    ...
    uint8_t state;
    int status;
    if(m_leftMotorInitialized) status = m_clientLeft.getAppState(state);
    ROS_INFO("State left motor : %i", status);

    if(m_rightMotorInitialized) status = m_clientRight.getAppState(state);
...
void DiffDriveController::cbSetSpeed(const geometry_msgs::PointConstPtr& speed)
{
    ...
    if(m_leftMotorInitialized) m_clientLeft.setSpeed(left); // RPM
    if(m_rightMotorInitialized) m_clientRight.setSpeed(right);
}
